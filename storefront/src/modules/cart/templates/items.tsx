...
const ItemsTemplate = ({ cart }: ItemsTemplateProps) => {
  ...
  return (
    <div>
      {/* Premium cart header */}
      <div className="px-8 py-6 bg-gradient-to-r from-green-600 to-emerald-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-green-100 text-sm mt-1">
                {items && `${items.length} ${items.length === 1 ? "premium artikel" : "premium artikelen"}`}
              </p>
            </div>
          </div>
          {items && items.length > 0 && (
            <div className="hidden sm:flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full">
              ...
              <span className="text-white font-bold text-sm">Premium Kwaliteit</span>
            </div>
          )}
        </div>
      </div>
